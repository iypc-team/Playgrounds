...
func createAxisScene() -> SCNScene {
    let axisScene = SCNScene()
    ...
    axisScene.rootNode.addChildNode(sphereNode)
    
    // Example geometry – a spinning box
    let box = SCNBox(width: 1, height: 1, length: 1, chamferRadius: 0)
    let boxNode = SCNNode(geometry: box)
    boxNode.name = "box"
    axisScene.rootNode.addChildNode(boxNode)
    ...
    axisScene.rootNode.addChildNode(xAxisNode)
    ...
    axisScene.rootNode.addChildNode(yAxisNode)
    ...
    axisScene.rootNode.addChildNode(zAxisNode)
    ...
    axisScene.rootNode.addChildNode(camNode)
    
    
    print("\(axisScene.rootNode.childNodes)\n")
    return axisScene
}
